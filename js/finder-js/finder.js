...
//var SERVICE_URL = 'http://localhost:8080/repository/api/ariadne/restp';
var ROOT_URL = 'http://ariadne.cs.kuleuven.be/finder/';
var START_DESCRIPTION = 0;
var END_DESCRIPTION = 650;
//var REPOSITORY_NAME = "AriadneNext Repository";
var REPOSITORY_NAME = "ARIADNE"; //you will find yours inside ariadne.properties file ex. var REPOSITORY_NAME = "AriadneNext Repository";
var THOUSAND_SEP = ',';
...
new Ajax.JSONRequest(SERVICE_URL, {
 ...
 $('search_results_index').update(' (#{start} - #{end} )'.interpolate({start: formatInteger(start + 1,THOUSAND_SEP), end: formatInteger(finalNumberResults,THOUSAND_SEP), total: formatInteger(result.nrOfResults,THOUSAND_SEP)}));
 pagination_show();
 }
 else {
                    ...
                    item.numbers.each(function(it2,idx2)
                                      ...
                                      it2.count = formatInteger(it2.count,THOUSAND_SEP);
                                                              ...
                                                              if(fld=="provider")
                                                              {
                                                                if (it2.val=="greenoer"){
                                                                     element.insert(Jaml.render('rbcriteria',it2));}
                                                                if (it2.val=="oerafrica"){
                                                                    element.insert(Jaml.render('rbcriteria',it2));}
                                                                if (it2.val=="ruforum"){
                                                                    element.insert(Jaml.render('rbcriteria',it2));}
                                                                if (it2.val=="digitalgreen"){
                                                                    element.insert(Jaml.render('rbcriteria',it2));}
                                                                if (it2.val=="faocapacityportal"){
                                                                    element.insert(Jaml.render('rbcriteria',it2));}
 ...
 Jaml.register('result',function(data){
    ...
    div({cls:'line'}, span("Keywords:"), keywordsToEmbed /*item.keywords*/)),
                                            div({cls:'floatright'},
 ...
 Jaml.register('rbcriteria', function(data)
               ...
               var label = data.val;
               if(data.val=="greenoer"){label = "Green OER";}//in future it will created through mapping for providers in order to have more readable labels
               if(data.val=="oerafrica"){label = "OER Africa";}
               if(data.val=="ruforum"){label = "RU Forum";}
               if(data.val=="digitalgreen"){label = "Digital Green";}
               if(data.val=="faocapacityportal"){label = "Fao Capacity Development Portal";}
